...
- **D30** — Phát hiện thật lúc deploy dev (tiếp theo D29, cùng 1 track test): transcript lưu thành công nhưng `summary_service.py` fail `invalid literal for int() with base 10: '1785754284.4878423'`. Root cause là lệch đơn vị thời gian, không phải lỗi mới của D29:
  - `record-service`'s `RecordingSession.started_at`/`ended_at` (`domain/models.py`) là `time.time()` — epoch **giây** (float, có phần thập phân).
  - `recording_event_service.py` (dòng cũ) chỉ `str(payload.started_at)` — stringify thẳng, không đổi đơn vị — rồi nhét vào `audio_info["started_at_ns"]`/`["ended_at_ns"]` qua `transcription_service.py`. Tên field ghi rõ `_ns` (nanosecond) nhưng giá trị thực chất là giây.
  - Cả 2 nơi tiêu thụ field này đều giả định epoch **nanosecond** integer đúng theo tên: `summary_service.py` (`int(...)`, rồi cộng dồn với offset segment × 1e9 để tính mốc tuyệt đối — công thức chỉ đúng nếu vế đầu là ns thật) và `room_service.py::get_audio_info()` (trả thẳng cho client dưới tên `started_at_ns`/`ended_at_ns` — client cũng sẽ hiểu sai đơn vị nếu không sửa, dù chưa ai báo lỗi vì có thể chưa có client nào parse strict).
  - Quy ước `_ns` này có từ trước `record-service` (khớp shape timestamp kiểu LiveKit webhook cũ) — không phải lỗi record-service tạo ra timestamp sai, mà là chỗ ghép nối (`recording_event_service.py`) quên đổi đơn vị khi chuyển từ nguồn mới (giây) sang field đã có hợp đồng cũ (nanosecond).
  - **Sửa tại đúng 1 điểm giao nhau** (`recording_event_service.py`, nơi `payload.started_at`/`ended_at` từ record-service lần đầu đi vào orchestrator) thay vì vá từng nơi tiêu thụ: `started_at=str(round(payload.started_at * 1_000_000_000))`, tương tự cho `ended_at`. Sửa 1 chỗ, tự động đúng cho cả `summary_service.py` lẫn `room_service.py::get_audio_info()` — không cần đụng gì ở 2 nơi tiêu thụ.
  - **Dữ liệu cũ đã lưu sai (trước khi patch này chạy) không tự sửa** — các track đã có `started_at_ns` sai đơn vị trong Postgres (giây, không phải ns) sẽ tiếp tục fail summary cho tới khi có track mới ghi đè (upsert theo `record_id`), hoặc cần 1 lần dọn dữ liệu tay nếu muốn summary đúng ngay cho các track cũ. Không phải lỗi cần fix thêm, chỉ là hệ quả tự nhiên của sửa tại nguồn thay vì sửa tại đích.
- **D31** — Phát hiện thật lúc deploy `agents` lên dev (không liên quan tới logic hotfix, thuần vấn đề toolchain/dependency): `recording_pb2.py`/`recording_pb2_grpc.py` (cả `agents` lẫn `record-service`) được generate bằng `protobuf`/`grpcio-tools` mới nhất hiện có lúc code (7.35.1 / grpcio-tools ứng với gencode yêu cầu `grpcio>=1.81.1`) — nhưng venv thật trên server dev/prod lại có `protobuf==6.33.5`/`grpcio==1.78.0` (cũ hơn), khiến cả 2 lần deploy `agents` đều crash ngay lúc import (`google.protobuf.runtime_version.VersionError`, rồi tới `grpc`'s tương tự `RuntimeError: ... depends on grpcio>=1.81.1`) — file gencode tự kiểm tra version runtime lúc import, cũ hơn là fail cứng.
  - Thử nâng `protobuf` lên 7.35.1 trên server để qua lỗi đầu tiên thì lộ ra vấn đề thứ 2: 2 gói khác trong dependency tree của `agents` (`googleapis-common-protos`, `opentelemetry-proto` — transitive qua LiveKit SDK, phần tracing/telemetry) khai báo cứng `protobuf<7.0`. Nâng protobuf để qua được `recording_pb2.py` lại phá compat 2 gói này — đánh đổi sai hướng (đổi 1 lỗi ồn ào lấy 1 lỗi âm thầm tiềm ẩn ở tầng telemetry).
  - **Sửa đúng hướng**: quay đầu vấn đề — hạ **gencode** xuống khớp đúng những gì server đã có sẵn và ổn định, thay vì nâng runtime lên theo gencode. Regenerate `recording_pb2.py`/`recording_pb2_grpc.py` (cả 2 bên `agents` và `record-service`, theo đúng `scripts/gen_proto.sh` mỗi bên) bằng toolchain ghim `protobuf==6.33.5` + `grpcio-tools==1.78.0` — khớp đúng phiên bản đã chạy ổn định trên server, không phải bản mới nhất có sẵn lúc code. Diff kết quả chỉ đổi version stamp trong 2 file gencode (`7,35,1`→`6,33,5`, tương tự cho grpc), không đổi gì về schema/wire format. Verify: import thành công ở đúng `protobuf==6.33.5`/`grpcio==1.78.0`, 14/14 test `record-service` vẫn pass (runtime mới hơn gencode vẫn qua được check, nên không phá gì ở `record-service`'s venv dù nó có bản protobuf khác).
  - **Chốt ceiling `protobuf<7.0` vào `requirements-agent.txt`** (trước đó `protobuf>=4.25.0` không có trần) — để lần cài venv mới nào sau này (server mới, CI, cài lại từ đầu) không lặp lại đúng vòng lặp trên do pip tự chọn bản mới nhất. `record-service/pyproject.toml` **không** cần trần tương tự — đã verify venv của nó không có `googleapis-common-protos`/`opentelemetry-proto` trong dependency tree nên không bị conflict, giữ nguyên `protobuf>=4.25` để không tự giới hạn không cần thiết.
  - **Nhắc lại nếu sau này cần sửa `recording.proto`**: chạy `scripts/gen_proto.sh` trong môi trường có ghim `protobuf<7.0` (không dùng venv toolchain mới nhất tuỳ ý) — nếu không sẽ tái diễn đúng vòng lặp lỗi này lần nữa. Không có gì tự động nhắc việc này ngoài chính ghi chú ở đây và comment trong `requirements-agent.txt`.
