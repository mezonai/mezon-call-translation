...
class OrchestratorClient:
    ...
    async def register_room(self, room_name: str, room_id: str) -> bool:
        ...
        Args:
            ...
            room_id: This worker's own stable UUID (audio-ingestion PLAN.md
                D27x -- generated by the caller, not the orchestrator; a new
                registration for the same room_name always supersedes
                whatever was registered before, no 409/conflict case exists
                anymore)
        ...
        try:
            ...
            if response.status_code in [200, 201]:
                self.logger.info(f"✅ Room '{room_name}' registered successfully with orchestrator (room_id: {room_id})")
                return True
            else:
                text = response.text
