...
class EventHandlers:
    ...
    def create_transcription_callback(self, participant_identity: str):
        """Factory to create callback for processing transcripts from Nemotron server"""
    ...
    async def manage_speaker_transcription(
        ...
        LiveKit track -> VAD processor -> WebSocket -> Nemotron server
        """
        ...
        # WebSocket client connects to Nemotron server
        ...
        try:
            ...
            async for event in stream:
                ...
                # Get batched chunks (5 chunks = 50ms) to send to Nemotron
