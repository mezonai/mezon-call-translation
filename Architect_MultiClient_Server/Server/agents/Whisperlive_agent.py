...
class WhisperLiveChatClient:
    ...
    def __init__(self, host, port, lang=None, model="small", use_vad=True, 
        ...
        # Remove transcript tracking - we'll send everything immediately
        self.last_sent_text = ""  # Track last sent text to avoid exact duplicates only
    ...
    def on_open(self, ws):
        ...
        # Send initial configuration (matching client.py)
        config = {
            "uid": self.uid,
    ...
    def on_message(self, ws, message):
        ...
        try:
            ...
            print(f"📨 Received from Whisper: {message_data}")
            ...
            # Handle transcription segments - SEND IMMEDIATELY
            if "segments" in message_data:
                self._process_segments_instant(message_data["segments"])
    ...
    def _process_segments_instant(self, segments):
        """Process transcription segments and send immediately to chat"""
        if not segments:
            return
        ...
        # Extract all text from segments and send immediately
        text_parts = []
        for seg in segments:
        ...
        if text_parts:
            ...
            # Only filter out exact duplicates to reduce spam
            if current_text.strip() and current_text.strip() != self.last_sent_text.strip():
                ...
                try:
                    if self.loop and not self.loop.is_closed():
                        ...
                        print(f"💬 Sent to chat: {current_text[:100]}{'...' if len(current_text) > 100 else ''}")
                except Exception as e:
...
async def entrypoint(ctx: agents.JobContext):
    ...
    def create_transcription_callback(participant_identity: str, participant_name: str):
        ...
        async def transcription_callback(text: str, segments: list):
            """Handle transcription results from Whisper Live and send to chat IMMEDIATELY"""
            if not text or not text.strip():
                return
            
            # Send ALL transcription results immediately to chat
    ...
    # Set agent name
    await ctx.room.local_participant.set_name("Whisper Live Chat Agent (Instant)")
    
    # Send welcome message to chat
    async def send_welcome_message():
        await asyncio.sleep(2)  # Wait for stable connection
        await send_chat_message(
            text="🤖 Whisper Live chat agent is ready! All transcriptions will appear instantly in chat.",
            participant_identity="agent",
            participant_name="Chat Agent"
        )
    
    asyncio.create_task(send_welcome_message())
    print("🎯 Whisper Live Chat Agent (Instant Mode) ready and waiting for participants...")
