...
- **Tripled LLM retry budget as a Gemini rate-limit workaround.** `SummaryService._call_llm` calls `tenacity.retry(stop=stop_after_attempt(max_attempts * 3), ...)` — the configured `SUMMARY_LLM_RETRY_COUNT` / `SUMMARY_LLM_FALLBACK_RETRY_COUNT` is multiplied by 3 before being handed to tenacity. The accompanying `WaitCustomStrategy` (`utils/retry_utils.py`) waits 1s within a group of 3 attempts, then 60s/70s/80s.../after each group of 3 — a deliberate but blunt way of giving Gemini's ~16k-tokens/min rate limit (429s) more room to clear before giving up, rather than a clean fix (e.g. actual client-side rate limiting/backpressure ahead of the call). See project memory `project_gemini_rate_limit_retry_workaround.md`.
