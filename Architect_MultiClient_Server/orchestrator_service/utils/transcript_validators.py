...
def validate_participant_identity(value: str) -> str:
    ...
    if not value or len(value) < VC.MIN_PARTICIPANT_ID_LENGTH:
        raise HTTPException(
            status_code=400, detail=f"Participant identity must be at least {VC.MIN_PARTICIPANT_ID_LENGTH} character(s)"
        )
    if len(value) > VC.MAX_PARTICIPANT_ID_LENGTH:
        raise HTTPException(
            status_code=400, detail=f"Participant identity must not exceed {VC.MAX_PARTICIPANT_ID_LENGTH} characters"
        )
    return value
...
def validate_confidence_range(min_confidence: float, max_confidence: float):
    ...
    if min_confidence > max_confidence:
        raise HTTPException(status_code=400, detail="min_confidence must be less than or equal to max_confidence")
...
ParticipantIdentityPath = Annotated[
    str,
    Path(
