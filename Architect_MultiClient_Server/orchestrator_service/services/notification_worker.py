...
class NotificationWorker:
    ...
    async def process_task(self, task: NotificationTask) -> bool:
        ...
        try:
            ...
            if not channel_id or not webhook_token:
                logger.error(
                    "❌ Missing notification config: channel_id or webhook_token not configured"
                )
                return False
    ...
    async def start(self) -> None:
        ...
        self._consumer_task = asyncio.create_task(
            self._consumer_loop(),
            name="notification-worker-consumer"
        )
        ...
        try:
            while self._running:
                try:
                    ...
                    tasks = await self._stream_service.read_tasks(
                        count=10,
                        block_ms=5000
                    )
                    ...
                    for task in tasks:
                        ...
                        if success:
                            ...
                            await self._stream_service.reject(
                                task,
                                error=error_msg,
                                retry=True
                            )
