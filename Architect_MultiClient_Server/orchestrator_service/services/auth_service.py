...
from orchestrator_service.services.postgresql.pg_token_blacklist_repository import (
    PgTokenBlacklistRepository,
    get_pg_token_blacklist_repository,
)
from orchestrator_service.services.postgresql.pg_user_permission_repository import (
    ...
    async def exchange_code_for_token(self, code: str, state: str) -> dict[str, Any]:
        try:
            ...
            userinfo_response = requests.get(
                self.oauth2_config.userinfo_url, headers={"Authorization": f"Bearer {access_token}"}, timeout=10
            )
