...
class AuthService:
    def __init__ (
        ...
        logger.info(f"  - Client ID: {'configured' if self.oauth2_config.client_id else 'NOT SET'}")
        logger.info(f"  - Client Secret: {'configured' if self.oauth2_config.client_secret else 'NOT SET'}")
    ...
    async def exchange_code_for_token(self, code: str, state: str) -> Dict[str, Any]:
        ...
        if not re.match(r'^[a-zA-Z0-9]{11}$', state):
        ...
        if not self.oauth2_config.client_id or not self.oauth2_config.client_secret:
        ...
        logger.debug(f"Client ID: {self.oauth2_config.client_id if self.oauth2_config.client_id else 'EMPTY'}...")
        logger.debug(f"Client Secret configured: {'YES' if self.oauth2_config.client_secret else 'NO'}")
        logger.debug(f"Redirect URI: {self.oauth2_config.redirect_uri}")
