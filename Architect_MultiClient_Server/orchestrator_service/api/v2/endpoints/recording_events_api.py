...
async def recording_events_endpoint(
    ...
    try:
        if event in _RECORDING_EVENTS:
            payload = RecordingEventRequest.model_validate(body)
            return await recording_event_service.handle_recording_event(payload)

        if event in _DERIVATIVE_EVENTS:
            payload = DerivativeEventRequest.model_validate(body)
            return await recording_event_service.handle_derivative_event(payload)
