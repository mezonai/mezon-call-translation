...
class SSEManager:
    ...
    async def disconnect_existing_appid(
        self, 
        channel_type: str, 
        context_key: str, 
        appid: str
        ...
        """
        async with self._lock:
            if (channel_type not in self.connection_appids or 
                context_key not in self.connection_appids[channel_type]):
                return False
            ...
            if existing_connection_id:
                ...
                if (channel_type in self.connection_queues and 
                    context_key in self.connection_queues[channel_type]):
    ...
    async def unregister_connection(
        ...
        async with self._lock:
            ...
            if (channel_type in self.connection_appids and 
                context_key in self.connection_appids[channel_type]):
    ...
    async def broadcast_message(
        ...
        """
        async with self._lock:
            if (channel_type not in self.connection_queues or 
                context_key not in self.connection_queues[channel_type]):
    ...
    async def has_active_connections(self, channel_type: str, context_key: str) -> bool:
        ...
        """
        async with self._lock:
            return (channel_type in self.connection_queues and 
                    context_key in self.connection_queues[channel_type] and 
                    len(self.connection_queues[channel_type][context_key]) > 0)
    ...
    async def cleanup(self):
        ...
        async with self._lock:
            total_connections = sum(
                len(connections)
                for contexts in self.connection_queues.values()
                for connections in contexts.values()
            )
