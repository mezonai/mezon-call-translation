...
class ChatExternalChannel:
    ...
    async def create_connection(
        ...
        if existing_disconnected:
            logger.info(
                f"[Chat External Channel] Closed existing connection for appid {appid}"
            )
    ...
    async def push_chat_event(
        ...
        if not await self.manager.has_active_connections(self.CHANNEL_TYPE, context_key):
            logger.warning(
                f"[Chat External Channel] No active bot connections, event may be lost"
            )
        ...
        logger.info(
            f"[Chat External Channel] Pushed event to {broadcast_count} bots "
            f"(room={room_name})"
        )
