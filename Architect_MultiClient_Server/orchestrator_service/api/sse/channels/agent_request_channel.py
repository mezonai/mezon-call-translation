...
class AgentRequestChannel:
    ...
    async def create_connection(
        ...
        if existing_disconnected:
            logger.info(
                f"[Agent Request Channel] Closed existing connection for agent {agent_id} "
                f"in context {context_key}"
            )
    ...
    async def send_request(
        ...
        if not await self.manager.has_active_connections(self.CHANNEL_TYPE, context_key):
            logger.warning(
                f"[Agent Request Channel] No active agents for context {context_key}, "
                "request may be lost"
            )
