"""
Shared Redis connection pool for the STT service process.
...
class RedisConnectionManager:
    """Singleton shared Redis pool for STT."""

    def __init__(self):
        self._config = get_config().redis
        self._pool: Optional[ConnectionPool] = None
        self._connected = False
        logger.debug("STT RedisConnectionManager initialized")
    ...
    async def connect(self) -> None:
        ...
        try:
            ...
            logger.info(
                f"✅ STT Redis shared pool: {self._config.host}:{self._config.port} "
                f"(max_connections={self._config.max_connections})"
            )
        except Exception as e:
            logger.error(f"✗ STT Redis pool failed: {e}")
            self._pool = None
            self._connected = False
    ...
    async def disconnect(self) -> None:
        if self._pool:
            ...
            self._pool = None
        self._connected = False
        logger.info("STT Redis connection pool closed")
