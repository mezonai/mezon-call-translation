...
| `AGENT_SOCKET_DIR` | no | `/tmp/mezon-agents` | Directory this agent listens on a per-room Unix domain control socket in (`internal/agentcontrol`), open from startup so a worker-manager that restarts later can reconnect to it (restart-recovery Phase 1-4, `mezon-sfu-migration-plan.md`). Must be the same value worker-manager itself is configured with — it's the one passing this through unchanged, not something to set independently per agent. |
| `AGENT_MAX_LIFETIME_SECONDS` | no | `10800` (3h) | Self-imposed upper bound on how long this agent runs before it shuts itself down gracefully (same path as SIGTERM) — tied to call/pricing plans. `0` disables it. Enforced agent-side, not by worker-manager watching a clock, precisely so it still holds even if worker-manager is down when the deadline hits or this agent was adopted (Phase 3) by an instance that never saw its real start time. Worker-manager sets this explicitly per spawn (`internal/workermanager/manager.go`'s `Start`), not via the passthrough list below — the plug-in point for a future per-event override once BE mezon sends a plan-derived value over NATS. |
| `AGENT_EMPTY_ROOM_GRACE_SECONDS` | no | `15` | How long to keep running after `participant_count` (mezon-sfu counts this agent same as any real user) drops to `1` — i.e. this agent is the only one left — before shutting itself down gracefully. `0` disables it. A grace period, not immediate, because `participant_count` also dips on the last human's own transient reconnect, not just a deliberate leave — cancelled the moment anyone rejoins. Deliberately independent of/much shorter than `AGENT_RECONNECT_*`'s own budget (~2-3 min worst case with defaults) — see `session.checkEmptyRoom`'s doc (`cmd/agent/main.go`) for why the two never compound. |
| `AGENT_CHAT_NAME` | no | `KOMU Agent` | Display name stamped on chat messages the agent posts into a room (the `send_chat_message` SSE agent-request → mezon-sfu `send_message`, encoded as a `signaling.RoomMessage`). |
| `AGENT_CHAT_AVATAR_URL` | no | `""` | Avatar URL stamped alongside `AGENT_CHAT_NAME` on posted chat messages — a public image URL the receiving chat UI renders; empty means the UI falls back to an initials tile. |
...
| `AGENT_SOCKET_DIR` | no | `/tmp/mezon-agents` | Restart-recovery Phase 1-4 (`mezon-sfu-migration-plan.md`). Read *and* used directly here (not just forwarded, unlike the passthrough vars below) — `Reconcile()` (`internal/workermanager/reconcile.go`) Dials into this directory on startup to re-adopt agents a previous instance left running. Must match every spawned agent's own copy (see the agent config table above) — same env var name, passed through unchanged, so this is automatic as long as nothing overrides it per-agent. |
| `WORKER_MANAGER_STATE_DIR` | no | `/tmp/mezon-agents` | Restart-recovery Phase 1-4. Where `registry.json` (`room_id → pid` mapping, `internal/workermanager/registry_store.go`) is persisted on every `Start`/`Stop`/`reap` and read back by `Reconcile()` at startup. Worker-manager's own, never passed to the agent (it has no reason to read this file). Production (systemd) should point this and `AGENT_SOCKET_DIR` at `StateDirectory=`/`RuntimeDirectory=`-managed paths instead of the `/tmp` dev default — see `deploy/systemd/`. |
| `AGENT_MAX_LIFETIME_SECONDS` | no | `10800` (3h) | Default agent lifetime, written into every spawn's `AGENT_MAX_LIFETIME_SECONDS` (see the agent config table above) — set explicitly per spawn (`manager.go`'s `Start`), not through the passthrough list below, since this is where a future per-event override plugs in. |

`worker-manager` also reads (and passes through unchanged to every spawned `agent`): `SFU_WS_URL`, `SFU_JWT_SECRET`, `AGENT_TOKEN_TTL_SECONDS`, all four `AGENT_RECONNECT_*` vars, `RECORD_SERVICE_*`, `WS_HOST`/`WS_PORT`, `STT_MAX_QUEUE_SIZE`, `ORCHESTRATOR_BASE_URL`, `INTERNAL_API_SECRET`, `TTS_*`, `AGENT_SOCKET_DIR`, `AGENT_EMPTY_ROOM_GRACE_SECONDS`, `AGENT_CHAT_NAME`, `AGENT_CHAT_AVATAR_URL`, and `LOG_LEVEL` (`internal/workermanager/config.go`'s `agentPassthroughEnvKeys` -- keep in sync with `internal/config.Config` when adding new agent env vars). Per-room `ROOM_ID`/`AGENT_USER_ID`/`AGENT_ROLE` come from the NATS event, not the environment.
