...
# - Server Vosk Service (STT)
# - Agents Service (LiveKit Agent + TTS)
...
# Service directories (updated to match actual project structure)
...
create_service_file \
    "server-vosk-service" \
    "Mezon Call Translation - Server Vosk STT Service" \
    "$SERVER_VOSK_DIR" \
    "$SERVER_VOSK_DIR/venv/bin/python -m uvicorn main:app --host 0.0.0.0 --port 8000"

# Agents Service
create_service_file \
    "agents-service" \
    "Mezon Call Translation - Agents Service (LiveKit Agent + TTS)" \
    "$AGENTS_DIR" \
    "$AGENTS_DIR/venv/bin/python main.py dev"
...
if [ "$ENABLE_SERVICES" = true ] && [ "$DRY_RUN" = false ]; then
    ...
    for service in mezon-server-vosk-service mezon-agents-service; do
...
if [ "$DRY_RUN" = true ]; then
    ...
    echo -e "  ${CYAN}sudo systemctl status mezon-server-vosk-service${NC}"
    echo -e "  ${CYAN}sudo systemctl status mezon-agents-service${NC}"
    echo ""
    echo -e "  # Start all services"
    echo -e "  ${CYAN}sudo systemctl start mezon-server-vosk-service mezon-agents-service${NC}"
    echo ""
    echo -e "  # Stop all services"
    echo -e "  ${CYAN}sudo systemctl stop mezon-server-vosk-service mezon-agents-service${NC}"
    echo ""
    echo -e "  # Restart all services"
    echo -e "  ${CYAN}sudo systemctl restart mezon-server-vosk-service mezon-agents-service${NC}"
    echo ""
    echo -e "  # Enable all services to start on boot"
    echo -e "  ${CYAN}sudo systemctl enable mezon-server-vosk-service mezon-agents-service${NC}"
    echo ""
    echo -e "  # Disable all services from starting on boot"
    echo -e "  ${CYAN}sudo systemctl disable mezon-server-vosk-service mezon-agents-service${NC}"
    echo ""
    echo -e "  # View live logs (all services)"
    echo -e "  ${CYAN}sudo journalctl -u mezon-server-vosk-service -u mezon-agents-service -f${NC}"
    echo ""
    echo -e "  # View logs for specific service"
    echo -e "  ${CYAN}sudo journalctl -u mezon-server-vosk-service -f${NC}"
    echo -e "  ${CYAN}sudo journalctl -u mezon-agents-service -f${NC}"
    echo ""
    echo -e "  # View recent logs (last 50 lines)"
    echo -e "  ${CYAN}sudo journalctl -u mezon-server-vosk-service -n 50${NC}"
    echo -e "  ${CYAN}sudo journalctl -u mezon-agents-service -n 50${NC}"
    echo ""
    echo -e "  # View logs since last boot"
    echo -e "  ${CYAN}sudo journalctl -u mezon-server-vosk-service -b${NC}"
    echo ""
fi
