...
> - **Cập nhật 2026-08-24 (thảo luận nợ kỹ thuật, chưa code)**: Rà lại toàn bộ các mục "còn mở"/nợ kỹ thuật của đợt migrate với team. Quyết định/kết luận chính: (1) **Data channel (B2) chốt hướng KHÔNG chờ SFU làm** — thay bằng 1 "conbot" join thẳng room chat Mezon, kiến trúc đang research, xem `mezon-sfu-migration-plan.md` mục 2.6b (còn mở câu hỏi: 1 session bot multiplex nhiều room hay mỗi room 1 session — quyết định process model, cần hỏi Mezon chat/bot API). (2) `kind="agent"` (A1.2/B2) — đồng ý minor, để sau. (3) TTS/uplink (B2) đã có sẵn quy trình test độc lập không cần API mới — xem `mezon-sfu-migration-plan.md` mục 2.5. (4) worker-manager restart gap (D3/mục 1 dòng 39 ở plan.md) — không phải "chấp nhận rủi ro", chỉ deprioritize vì `Stop()` thật tế chỉ trigger sau khi room đã trống. (5) `jwt_secret` (B1) — dev dùng tạm, long-term sẽ lấy qua conbot. (6) `room_name`/`room_id` (2.6 ở plan.md) — giải thích lại + có bằng chứng code cụ thể (`agent_request_channel.py::get_context_key`). (7) Admin API (A6) — kỳ vọng giải quyết qua conbot. (8) Event LiveKit cũ (A7.4) — đóng, team xác nhận không cần theo dõi.
...
| # | Chức năng dùng | Cách dùng hiện tại | mezon-sfu | Ghi chú |
|---|---|---|---|---|
| A6.1 | `client.room.list_participants()` | `livekit_client.py:295,326`, `room_registry_api.py:158` | ❌ Chưa có | Không có REST/gRPC admin API nào, chỉ có WS signaling. **[MỚI 08-19]** Không có "list", nhưng đã có 2 hành động moderation qua WS message `participant_action` (`{token: <jwt hành động riêng>}`): mute/kick 1 participant theo `user_id` — xem `mezon-sfu/CLAUDE.md` mục 4.1. **[Cập nhật 2026-08-24]** Kỳ vọng sẽ giải quyết được qua conbot (2.6b) — participant info chủ yếu lấy được qua chính room chat Mezon mà bot join, không cần admin API riêng của SFU |
| A6.2 | `get_participant_detail` (tracks, permission, kind, state...) | `livekit_client.py:313-391` | ❌ Chưa có | **[Cập nhật 2026-08-24]** Tương tự A6.1, kỳ vọng giải quyết qua conbot |
...
| A7.4 | ~30 loại event khác của LiveKit (room_started/ended, track_muted, egress_*, participant_active...) — hiện bị ignore hết ngoại trừ A7.2/A7.3 | `webhook_handler.py:45-55` | ❌ Chưa có | Không cấp thiết. Riêng "room_started/ended" vẫn chưa có event NATS tương đương — chỉ suy ra gián tiếp qua event `join` đầu tiên/`leave` cuối cùng của room, không có event "room" độc lập. **[Cập nhật 2026-08-24, đóng]** Team xác nhận: toàn bộ event LiveKit cũ được thay thế hoàn toàn bằng NATS event + WS signaling event hiện có; phần lớn 30 event kia vốn không dùng tới, không phải gap thật — không cần theo dõi thêm mục này |
...
- ~~Không có JWT~~ → **Đã có (A1.1)**: câu hỏi giờ đổi thành — **ai cấp `jwt_secret` dùng chung cho mezon-call-translation để tự sinh token** (giống việc trước đây orchestrator giữ `LIVEKIT_API_KEY/SECRET`)? Secret này có tách riêng theo môi trường (dev/staging/prod) không? **[Cập nhật 2026-08-24]** Dev hiện đang được cấp tạm để dùng. Long-term: sẽ đổi hướng, **lấy token qua conbot** (xem B2/2.6b ở `mezon-sfu-migration-plan.md`) thay vì tự sinh bằng shared secret — phụ thuộc trực tiếp vào kiến trúc conbot được chốt trước.
